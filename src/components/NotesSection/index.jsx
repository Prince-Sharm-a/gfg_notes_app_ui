...
export const NotesSection=memo(({data})=>{
    ...
    const handleDeleteClick = (note) =>{
        dispatch(setDeletedNotes([...deletedNotes,note]))
        dispatch(move_to_trash(note.id));
        ...
        <div className={`notes-section ${isNoteSecActive? '' : 'hide'}`}>
            {
                ...
                <div className="notes-card" key={note?.id}>
                    ...
                    <button className="absolute right-2 bottom-1" onClick={()=>handleDeleteClick(note)}><DeleteOutlineIcon/></button>
                </div>
                ))
            }
        </div>
